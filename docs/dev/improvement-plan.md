...
5. **Dependency health**: GitHub still reports 26 Dependabot alerts on `main` (6 high, 16 moderate, 4 low) despite the #131 `overrides` pass — triage the remainder (see github.com/jspsych/metadata/security/dependabot) and revisit the `overrides` block once transitive deps catch up. Behind-major stack to schedule: React 18→19, Vite 5→7, Jest 29→30, ESLint 8→9.
6. Housekeeping: remove vestigial `husky` devDep (no `.husky/`); fix root `build` script's ignored `--platform=node`; add `cache: npm` to release.yml's setup-node; expand the 2-line root README with install/usage + links into `docs/`.
