...
The docs site lives in [`website/`](./website) and is built with [Docusaurus](https://docusaurus.io/) on the shared [jsPsych docs theme](https://github.com/jspsych/jspsych-docs-theme). Once deployed it will be available at **https://metadata.jspsych.org**, where the homepage is the metadata **web wizard** itself (built from [`packages/frontend`](./packages/frontend) and embedded at the site root).

It covers:

- **Introduction** — [what Psych-DS is and how the tools work](./website/docs/introduction.md), plus the wizard steps.
...
`npm start` and `npm run build` automatically run `npm run build:wizard` first, which builds the wizard from `packages/frontend` into `website/static/wizard-app/` (gitignored) so the homepage can embed it.
